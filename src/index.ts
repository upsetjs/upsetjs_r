...
declare const HTMLWidgets: {
  shinyMode: boolean, viewerMode: boolean,
...
function fixCombinations(combinations: GenerateSetCombinationsOptions | ISetCombinations<any> | undefined, sets: ISets<any>) {
  if (
    !combinations ||
  ...
  return asCombinations(
    ...
    "composite",
    (s: any) => s.setNames.map((si: string) => lookup.get(si))
        .filter(Boolean)
  );
}
...
HTMLWidgets.widget({
  ...
  factory(el, width, height) {
    ...
    function fixProps(props: UpSetProps<any> & { interactive?: boolean }, delta: any) {
      ...
      if (
        typeof delta.selection === "string" ||
        Array.isArray(delta.selection)
      ) {
